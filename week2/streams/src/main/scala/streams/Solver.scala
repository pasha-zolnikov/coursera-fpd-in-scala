...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    (b legalNeighbors) map (ln => (ln._1, ln._2 :: history)) toStream
    }
    ...
    def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
      ...
      neighbors filter (x => !explored(x._1))
    }
    ...
    def from(initial: Stream[(Block, List[Move])],
      ...
      if (initial.isEmpty) Stream.empty
        ...
        val more = {for {
          (b, m) <- initial
        } yield newNeighborsOnly(neighborsWithHistory(b, m), explored)}.flatten

        val moreExp = more.unzip._1

        initial #::: from(more, explored ++ moreExp)
      }
    }
